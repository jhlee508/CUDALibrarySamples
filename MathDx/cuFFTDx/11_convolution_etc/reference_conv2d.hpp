...
namespace example {
    ...
    namespace detail {
        // 2D cuFFT plan builder for convolution
        // dims = {x, y} with y being the contiguous (fastest) dimension
        inline auto get_cufft_convolution_plans_2d(int x, int in_x, int out_x,
                                                   int y, int in_y, int out_y,
                                                   int batches,
                                                   bool is_real_convolution,
                                                   bool is_double_precision,
                                                   bool is_forward_convolution,
            ...
            int rank = 2;
            int dims[] = {x, y}; // row-major: last dim (y) is contiguous

            int istride = 1;
            int idist = in_x * in_y;

            int ostride = 1;
            int odist = out_x * out_y;

            int n_batches = batches;

            CUFFT_CHECK_AND_EXIT(cufftPlanMany(&plan_front, rank, dims,
                                               /*inembed*/  nullptr, istride, idist,
                                               /*onembed*/  nullptr, ostride, odist,
                                               cufft_front_enum, n_batches));
            CUFFT_CHECK_AND_EXIT(cufftPlanMany(&plan_back, rank, dims,
                                               /*inembed*/  nullptr, ostride, odist,
                                               /*onembed*/  nullptr, istride, idist,
                                               cufft_back_enum, n_batches));
        ...
        cufftResult execute_cufft(Args&&... args) {
            cufftResult status;
            if constexpr (IsReal && IsForward && IsDouble) {
                status = cufftExecD2Z(std::forward<Args>(args)...);
            } else if constexpr (IsReal && IsForward && !IsDouble) {
    ...
    // 2D cuFFT reference convolution
    // IsReal: real-convolution (R2C/C2R) vs complex (C2C)
    // IsForward: true => forward conv (front=Forward, back=Inverse), false => inverse conv
    // LoadFunctor/FilterFunctor/StoreFunctor: same semantics as 3D version
    template<bool IsReal, bool IsForward, class LoadFunctor, class FilterFunctor, class StoreFunctor, typename IOType>
    auto cufft_2d_convolution(int          fft_size_x,
                              int          fft_size_y,
                              int          batches,
                              IOType*      input_dx,
                              IOType*      output_dx,
        ...
        using inter_type       = std::conditional_t<IsReal && !IsForward, precision, cufftdx::complex<precision>>;
        using cufft_inter_type = cufft_value_type_t<inter_type>;

        constexpr bool is_r2c_conv = is_real_conv && is_forward_conv;
        constexpr bool is_c2r_conv = is_real_conv && !is_forward_conv;

        int x_input_length  = fft_size_x;
        int y_input_length  = is_c2r_conv ? (fft_size_y / 2 + 1) : fft_size_y;

        int x_output_length = fft_size_x;
        int y_output_length = is_r2c_conv ? (fft_size_y / 2 + 1) : fft_size_y;

        auto input  = reinterpret_cast<cufft_type*>(input_dx);
        auto output = reinterpret_cast<cufft_type*>(output_dx);

        cufft_inter_type* inter = nullptr;
        const auto conv_length = x_output_length * y_output_length;
        ...
        int flat_input_size  = x_input_length  * y_input_length;
        int flat_output_size = x_output_length * y_output_length;

        auto cufft_plans = detail::get_cufft_convolution_plans_2d(
            fft_size_x, x_input_length, x_output_length,
            fft_size_y, y_input_length, y_output_length,
            batches, is_real_conv, is_double, is_forward_conv, stream);
        ...
        auto cufft_execution = [&](cudaStream_t /* s */) {
            constexpr bool use_preprocess = !std::is_same_v<LoadFunctor, example::identity>;
            if constexpr (use_preprocess) {
                thrust::transform(execution_policy,
                                  input, input + batches * flat_input_size,
                                  output, LoadFunctor{});
            }
            CUDA_CHECK_AND_EXIT(cudaDeviceSynchronize());

            auto cufft_res = detail::execute_cufft<is_real_conv, is_forward_conv, is_double>(
                                cufft_plan_front,
                                use_preprocess ? output : input,
                                inter);
            CUFFT_CHECK_AND_EXIT(cufft_res);
            CUDA_CHECK_AND_EXIT(cudaDeviceSynchronize());

            if constexpr (!std::is_same_v<FilterFunctor, example::identity>) {
                thrust::transform(execution_policy,
                                  inter, inter + batches * flat_output_size,
                                  inter, FilterFunctor{});
            }
            CUDA_CHECK_AND_EXIT(cudaDeviceSynchronize());

            cufft_res = detail::execute_cufft<is_real_conv, !is_forward_conv, is_double>(
                            cufft_plan_back, inter, output);
            CUFFT_CHECK_AND_EXIT(cufft_res);
            CUDA_CHECK_AND_EXIT(cudaDeviceSynchronize());

            if constexpr (!std::is_same_v<StoreFunctor, example::identity>) {
                thrust::transform(execution_policy,
                                  output, output + batches * flat_input_size,
                                  output, StoreFunctor{});
            }
            CUDA_CHECK_AND_EXIT(cudaDeviceSynchronize());
        };
        ...
        auto time = example::measure_execution_ms(
            cufft_execution, warm_up_runs, perf_runs, stream);
