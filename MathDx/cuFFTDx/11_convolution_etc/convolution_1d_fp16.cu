...
// cufft_conv_1d(T *input, T *output, const unsigned int bs, cudaStream_t
// stream) {
//   using complex_type = typename example::make_cufft_compatible<typename
//   FFT::value_type>::type;
...
//       std::is_same_v<complex_type, cufftComplex> ? CUFFT_C2C : CUFFT_Z2Z,
//       bs));
//   CUFFT_CHECK_AND_EXIT(cufftPlan1d(
...
//           cufftExecC2C(plan_forward, cufft_input, cufft_output,
//           CUFFT_FORWARD));
//     } else if constexpr (std::is_same_v<complex_type, cufftDoubleComplex>) {
//       CUFFT_CHECK_AND_EXIT(
//           cufftExecZ2Z(plan_forward, cufft_input, cufft_output,
//           CUFFT_FORWARD));
//     }
...
template <class FFT, class IFFT, bool InputInRRIILayout = false,
          bool OutputInRRIILayout = false>
__launch_bounds__(FFT::max_threads_per_block) __global__
    void convolution_fp16_kernel(typename FFT::value_type *input,
  ...
  example::io<FFT>::load<InputInRRIILayout>(input, thread_data, local_fft_id);
  ...
  // Save results
  example::io<FFT>::store<OutputInRRIILayout>(thread_data, output,
                                              local_fft_id);
}

template <class FFT, class IFFT, class T>
example::fft_results<T> cufftdx_conv_1d_fp16(T *input, T *output,
  ...
  CUDA_CHECK_AND_EXIT(cudaFuncSetAttribute(
      convolution_fp16_kernel<FFT, IFFT>,
  ...
  static const size_t total_fft_size =
      bs_div_fpb * FFT::ffts_per_block / FFT::implicit_type_batching * fft_size;
  ...
  std::vector<complex_type> output_host(
      total_fft_size, {__half2{std::numeric_limits<__half>::quiet_NaN(),
                               std::numeric_limits<__half>::quiet_NaN()},
                       __half2{std::numeric_limits<__half>::quiet_NaN(),
                               std::numeric_limits<__half>::quiet_NaN()}});
...
template <unsigned int Arch> void conv_1d() {
  ...
  for (size_t i = 0; i < input_size; i++) {
    ...
    host_data[i] = complex_type{__half2{v1, -v1}, __half2{v2, -v2}};
  }

#ifdef CUFFTDX_EXAMPLE_DETAIL_DEBUG_CONV_1D
  std::cout << "input:\n";
  for (size_t i = 0; i < input_size; i++) {
    std::cout << __half2float(host_data[i].x.x) << " "
              << __half2float(host_data[i].x.y) << std::endl;
  }
  for (size_t i = 0; i < input_size; i++) {
    std::cout << __half2float(host_data[i].y.x) << " "
              << __half2float(host_data[i].y.y) << std::endl;
  }
#endif
  ...
  // cuFFTDx convolution
  auto cufftdx_results = cufftdx_conv_1d_fp16<FFT, IFFT>(
      input, output, batches / ffts_per_block, stream);

  // cuFFT convolution as reference
  // auto cufft_results = cufft_conv_1d<FFT, fft_size>(input, output, batches,
  // stream);
  ...
  for (size_t i = 0; i < input_size; i++) {
    std::cout << __half2float(cufftdx_results.output[i].x.x) << " "
              << __half2float(cufftdx_results.output[i].x.y) << std::endl;
  }
  for (size_t i = 0; i < input_size; i++) {
    std::cout << __half2float(cufftdx_results.output[i].y.x) << " "
              << __half2float(cufftdx_results.output[i].y.y) << std::endl;
  }

  // std::cout << "output of cuFFT:\n";
  // for (size_t i = 0; i < cufft_results.output.size(); i++) {
  //   std::cout << cufft_results.output[i].x << " " <<
  //   cufft_results.output[i].y
  //             << std::endl;
  // }
#endif
